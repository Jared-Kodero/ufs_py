...
class UniqueKeyLoader(yaml.SafeLoader):
    ...
    def construct_mapping(self, node, deep=False):
        ...
        for key_node, value_node in node.value:
            ...
            if key in mapping:
                sys.exit("ERROR: You have defined the key:" + key + " multiple times")
            mapping.append(key)
...
def check_time_units(file_name, key_name, time_units):
    ...
    if time_units not in valid:
        sys.exit(
            "ERROR: "
            + time_units
            + " is not a valid unit. Check your "
            + key_name
            + " entry for file:"
            + file_name
        )
...
def check_freq(file_name, freq, freq_units):
    ...
    if freq < -1:
        sys.exit(
            "ERROR: freq needs to greater than -1. Check your freq entry for file:"
            + file_name
        )
...
def check_required_diag_files_key(diag_file):
    ...
    if "file_name" not in diag_file:
        sys.exit("ERROR: file_name is a required key!")
    if "freq" not in diag_file:
        sys.exit(
            "ERROR: freq is a required key! Add it for file:" + diag_file["file_name"]
        )
    if "freq_units" not in diag_file:
        sys.exit(
            "ERROR: freq_units is a required key! Add it for file:"
            + diag_file["file_name"]
        )
    if "time_units" not in diag_file:
        sys.exit(
            "ERROR: time_units is a required key! Add it for file:"
            + diag_file["file_name"]
        )
    if "unlimdim" not in diag_file:
        sys.exit(
            "ERROR: unlimdim is a required key! Add it for file:"
            + diag_file["file_name"]
        )
...
def check_new_file_freq(diag_file):
    ...
    if "new_file_freq" in diag_file:
        if "new_file_freq_units" not in diag_file:
            sys.exit(
                "ERROR: new_file_freq is present, but not new_file_freq_units. Check you entry for file:"
                + diag_file["file_name"]
            )
        if diag_file["new_file_freq"] < 1:
            sys.exit(
                "ERROR: new_file_freq needs to be greater than 0. Check your new_file_freq for "
                + diag_file["file_name"]
            )
        check_time_units(
            diag_file["file_name"],
            "new_file_freq_units",
            diag_file["new_file_freq_units"],
        )
    else:
        if "new_file_freq_units" in diag_file:
            sys.exit(
                "ERROR: new_file_freq_units is present, but not new_file_freq. Check your entry for file:"
                + diag_file["file_name"]
            )
...
def check_file_duration(diag_file):
    ...
    if "file_duration" in diag_file:
        if "file_duration_units" not in diag_file:
            sys.exit(
                "ERROR: file_duration is present, but not file_duration_units. Check you entry for file:"
                + diag_file["file_name"]
            )
        if diag_file["file_duration"] < 1:
            sys.exit(
                "ERROR: file_duration_units needs to be greater than 0. Check your file_duration_units for "
                + diag_file["file_name"]
            )
        check_time_units(
            diag_file["file_name"],
            "file_duration_units",
            diag_file["file_duration_units"],
        )
    else:
        if "file_duration_units" in diag_file:
            sys.exit(
                "ERROR: file_duration_units is present, but not file_duration. Check your entry for file:"
                + diag_file["file_name"]
            )
...
def check_start_time(diag_file):
    ...
    if "start_time" not in diag_file:
        return
    if "file_duration" not in diag_file:
        sys.exit(
            "ERROR: file_duration is needed if start_time is present. Check your entry for file:"
            + diag_file["file_name"]
        )
...
def check_sub_region(diag_file):
    ...
    if "sub_region" not in diag_file:
        return
    ...
    if "grid_type" not in sub_region:
        sys.exit(
            "ERROR: grid_type is required if defining a sub_region. Add it your file:"
            + diag_file["file_name"]
        )
...
def check_required_diag_field_key(diag_field, file_name):
    ...
    if "var_name" not in diag_field:
        sys.exit(
            "ERROR: var_name is a required field! Check your var_name in file: "
            + file_name
        )
    if "module" not in diag_field:
        sys.exit(
            "ERROR: module is a required field! Add it for variable:"
            + diag_field["var_name"]
            + " in file: "
            + file_name
        )
    if "reduction" not in diag_field:
        sys.exit(
            "ERROR: reduction is a required field! Add it for variable:"
            + diag_field["var_name"]
            + " in file: "
            + file_name
        )
    if "kind" not in diag_field:
        sys.exit(
            "ERROR: kind is a required field! Add it for variable:"
            + diag_field["var_name"]
            + " in file: "
            + file_name
        )
...
def check_reduction(diag_field, file_name):
    ...
    if "diurnal" in reduction:
        if int(reduction[7:9]) < 0:
            sys.exit(
                "ERROR: The number of diurnal samples in "
                + reduction
                + " for variable:"
                + diag_field["var_name"]
                + " and file:"
...
with open(in_diag_table) as fl:
    ...
    if "title" not in my_table:
        sys.exit("ERROR: title is a required key!")
    if "base_date" not in my_table:
        sys.exit("ERROR: base_date is a required key!")
